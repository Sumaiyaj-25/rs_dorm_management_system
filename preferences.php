...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (
        ...
        $check = $pdo->prepare(
            'SELECT PreferenceID FROM preferences WHERE Student_ID = ?'
        );
        ...
        if ($check->fetch()) {
            ...
            $update = $pdo->prepare(
                'UPDATE preferences
                 SET Cleanliness = ?, NoiseTolerance = ?, StudyHabit = ?, SleepingHabit = ?, Others = ?
                 WHERE Student_ID = ?'
            );
            $update->execute([$cleanliness, $noise, $study, $sleep, $others, $student_id]);
            ...
            $insert->execute([$student_id, $cleanliness, $noise, $study, $sleep, $others]);
        }
...
$stmt = $pdo->prepare(
    'SELECT Cleanliness, NoiseTolerance, StudyHabit, SleepingHabit, Others
     FROM preferences
     WHERE Student_ID = ?'
);
...
<head>
    <meta charset="UTF-8">
    <title>My Preferences — Roommate Recommendation</title>
...
<div class="page">
    ...
    <p class="dashboard-subtitle">
        These answers are used to calculate a compatibility score
    ...
    <form class="form-card" method="POST">
        <label>
            ...
            <select name="cleanliness" required>
                ...
                <?php foreach ($cleanliness_options as $opt): ?>
                    <option value="<?= htmlspecialchars($opt) ?>"<?= selected_if($current['Cleanliness'] ?? '', $opt) ?>>
        ...
        <label>
            ...
            <select name="noise_tolerance" required>
                ...
                <?php foreach ($noise_options as $opt): ?>
                    <option value="<?= htmlspecialchars($opt) ?>"<?= selected_if($current['NoiseTolerance'] ?? '', $opt) ?>>
                        <?= htmlspecialchars($opt) ?>
                    </option>
                <?php endforeach; ?>
            </select>
        ...
        <label>
            ...
            <select name="study_habit" required>
                ...
                <?php foreach ($study_options as $opt): ?>
                    <option value="<?= htmlspecialchars($opt) ?>"<?= selected_if($current['StudyHabit'] ?? '', $opt) ?>>
                        <?= htmlspecialchars($opt) ?>
                    </option>
                <?php endforeach; ?>
            </select>
        ...
        <label>
            Sleeping Habit
            <select name="sleeping_habit" required>
                <option value="">Select an option</option>
                <?php foreach ($sleep_options as $opt): ?>
                    <option value="<?= htmlspecialchars($opt) ?>"<?= selected_if($current['SleepingHabit'] ?? '', $opt) ?>>
                        <?= htmlspecialchars($opt) ?>
                    </option>
                <?php endforeach; ?>
            </select>
        ...
        <label>
            ...
            <textarea name="others" rows="3" placeholder="e.g. Non-smoker, plays music in the evening..."><?= htmlspecialchars($current['Others'] ?? '') ?></textarea>
        </label>
